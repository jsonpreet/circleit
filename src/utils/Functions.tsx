export function timeStampToTimeAgo(p_timeStampNanoSeconds: number) {
...
export const dateFormat = (p_timeStampNanoSeconds: number) => {
  const milliseconds = p_timeStampNanoSeconds / 1000000;
...
export const getIsCircle = (circle: any) => {
...
export function abbreviateNumber(value: number, decimals: number, toUSD: number) {
  let shortValue;
...
export function nanosToUSDNumber(nanos: number) {
  return nanos / 1e9;
}

export function formatUSD(num: string, decimal : number) {
  let formatUSDMemo;
...
export const getAttachmentsClass = (attachments : number, isNew = false) => {
  if (attachments === 1) {
    return {
