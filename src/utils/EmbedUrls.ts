export function youtubeParser(url: string): string | boolean {
...
export function mousaiParser(url: string): string | boolean {
  const regExp = /^.*mousai\.stream\/((album|playlist|track)\/[0-9]+(\/[a-z0-9-_,]+){1,2}?)(?=(\/embed|$))/;
  const match = url.match(regExp);
  return match ? match[1] : false;
}

export function constructMousaiEmbedURL(url: URL): string {
...
export function constructTwitchEmbedURL(url: URL): any {
  const twitchParsed = twitchParser(url.toString());
  return twitchParsed ? `https://${twitchParsed}` : "";
}

export function getEmbedURL(embedURL: string) : string | undefined {
  if (!embedURL) {
    return;
  }
  let url;
  ...
  if (isTwitchFromURL(url)) {
    return constructTwitchEmbedURL(url).map((embedURL: string) => (embedURL ? embedURL + `&autoplay=false&parent=` : ""))
  }
  return;
}

export function isVimeoLink(link: URL): boolean | string {
  try {
...
export function isVimeoFromURL(url: URL) : boolean | string {
  const pattern = /\bvimeo\.com$/;
  return pattern.test(url.hostname);
}

export function isYoutubeLink(link: string): boolean | string {
  try {
    const url = new URL(link);
...
export function isYoutubeFromURL(url: URL) : boolean | string {
  const patterns = [/\byoutube\.com$/, /\byoutu\.be$/];
  return patterns.some((p) => p.test(url.hostname));
}

export function isMousaiLink(link: URL): boolean | string {
  try {
    const url = new URL(link);
...
export function isMousaiFromURL(url: URL) : boolean | string {
...
export function isValidMousaiEmbedURL(link: string): boolean {
